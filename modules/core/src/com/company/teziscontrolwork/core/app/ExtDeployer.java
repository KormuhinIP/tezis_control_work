package com.company.teziscontrolwork.core.app;
...
@ManagedBean("tezis_control_work_ExtDeployer")
public class ExtDeployer extends AbstractDeployer implements ExtDeployerMBean {
    ...
    protected void checkForFirstInit() {
        ...
        try {
            ...
            List<Config> configs = em.createQuery("select c from sys$Config c where c.name='tezis_control_work.initDefaultInFirstStart'",
                ...
                if (StringUtils.isNotBlank(edition) && Integer.parseInt(edition) > 1) {
                    ...
                    if (initDefaultInFirstStart == null) {
                        try {
                            ...
                            config.setName("tezis_control_work.initDefaultInFirstStart");
                        ...
                        try {
                            login();
                            em.createQuery("update sys$Config c set c.value='false' where c.name='tezis_control_work.initDefaultInFirstStart'",
    ...
    protected void executeInitScripts() {
        ...
        File folderDB = new File(dbDirPath + "/50-tezis_control_work/init/" + dbmsType);
